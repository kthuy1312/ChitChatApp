...
export const createConversation = async (req, res) => {
  try {
    ...
    await conversation.populate([
      {
        path: "participants.userID",
        select: "displayName avatarUrl offlineAt username",
      },
    ...
    //lọc tn ghim đối với ng đã clear conversation
    let filteredPinned = conversation.pinnedMessages || [];
    const clearedRecord = conversation.clearedAt?.find(
      (c) => c.userId.toString() === userId.toString(),
    );

    if (clearedRecord) {
      filteredPinned = filteredPinned.filter((p) => {
        return new Date(p.createdAt) > new Date(clearedRecord.timestamp);
      });
    }
...
export const getMessages = async (req, res) => {
  try {
    ...
    const clearData = conversation?.clearedAt.find(
      (c) => c.userId.toString() === userId.toString(),
    );
  ...
  try {
    ...
    if (!isParticipant) {
      return res.status(403).json({
        message: "Bạn không có quyền thay đổi theme cho cuộc trò chuyện này",
      });
    }
...
export const markAsSeen = async (req, res) => {
  try {
    ...
    if (last.senderId.toString() === userId) {
      return res.status(200).json({
        message: "tin nhắn được gửi bởi sender nên kh cần mark as seen",
      });
    }
...
export const setNickname = async (req, res) => {
  try {
    ...
    if (!targetId) {
      return res
        .status(404)
        .json({ message: "Thiếu thông tin người được đặt biệt danh" });
    }

    if (nickname && nickname.length > 30) {
      return res
        .status(400)
        .json({ message: "Biệt danh không được quá 30 ký tự" });
    }
...
export const addGroupMember = async (req, res) => {
  try {
    ...
    if (!userIds || userIds.length === 0) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin người cần thêm" });
    }
    ...
    //populate thêm
    const populatedUsers = await User.find({ _id: { $in: validUsers } }).select(
...
export const removeMember = async (req, res) => {
  try {
    ...
    const adminId = (
      convo.participants[0].userID || convo.participants[0]
    ).toString();
    ...
    if (adminId === userId) {
      return res
        .status(400)
        .json({ message: "Bạn không thể tự gỡ chính mình" });
    }
