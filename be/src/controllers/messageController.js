...
export const sendDirectMessage = async (req, res) => {
  try {
    ...
    // khi gửi message mới thì bỏ id ng nhận và người gửi khỏi hiddenFor nếu có - để hiển thị lại conver đó
...
export const unsendMessage = async (req, res) => {
  try {
    ...
    if (message.senderId.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ message: "Chỉ người gửi mới có quyền thu hồi tin nhắn" });
    }
  ...
  try {
    ...
    if (!isMember) {
      return res.status(403).json({
        message: "Bạn không có quyền ghim tin nhắn cho cuộc trò chuyện này",
      });
    }
    ...
    if (message.isUnsent) {
      return res
        .status(400)
        .json({ message: "Không ghim tin nhắn đã thu hồi" });
    }
    ...
    return res.status(200).json({
      message: isPinned
        ? "Bỏ ghim tin nhắn thành công"
        : "Ghim tin nhắn thành công",
    });
  } catch (error) {
...
export const toggleReaction = async (req, res) => {
  try {
    ...
    if (message.isUnsent) {
      return res
        .status(400)
        .json({ message: "Không thể react tin đã thu hồi" });
    }
