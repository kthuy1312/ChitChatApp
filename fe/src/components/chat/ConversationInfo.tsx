...
const ConversationInfo = ({
  ...
  const handleTheme = async (theme: string) => {
    try {
      const themeLabel =
        themeInfo[theme as keyof typeof themeInfo]?.label || theme;
  ...
  const handleSetNickname = async (userId: string) => {
    try {
      ...
      await useChatStore.getState().setNickname(chat._id, nickname, userId);
  ...
  //delete conversation
  const handleDelete = async (converId: string) => {
    try {
      await clearConversation(converId);
  ...
  const existingIds = chat.participants.map((p: any) => (p.userID || p)._id);

  const userResults = friends.filter((f: any) => !existingIds.includes(f._id));

  //search member
  const keyword = searchUser.trim().toLowerCase();

  const filteredUsers = userResults.filter((f: any) =>
    (f.displayName || f.username || "").toLowerCase().includes(keyword),
  );
  ...
  // Kiểm tra xem cuộc trò chuyện có bị hạn chế không
  const isRestricted = chat.participants?.some((p: any) => p.isRestricted) ?? false;
  ...
  return (
    <>
        ...
        {view === "info" && (
          <div className="flex flex-col overflow-hidden bg-background">
            <div className="relative flex flex-col items-center p-7 bg-gradient-to-b from-primary/10 via-background to-background border-b border-border/50">
              ...
              <div className="text-center mt-3 space-y-1">
                ...
                {chat.type === "direct" ? (
                  !isRestricted && (
                    <p
                      className={`text-sm font-semibold px-3 py-1 rounded-full inline-block ${isOnline ? "bg-green-500/10 text-green-600" : "bg-muted text-muted-foreground"}`}
                    >
                      {statusText}
                    </p>
                  )
            ...
            <div className="p-4 space-y-6 overflow-y-auto beautiful-scrollbar max-h-[60vh]">
              {/* Tùy chỉnh */}
              {(chat.type === "group" ||
                (chat.type === "direct" && !isOtherRestricted)) && (
                ...
                {Object.entries(chatThemes).map(([key]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex items-center justify-between p-4 rounded-xl transition-all border-2 ${
                        isSelectedTheme
      ...
      {openDeleteDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-background rounded-2xl p-6 w-[320px] shadow-xl border border-border space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Hành động này không thể hoàn tác. Toàn bộ tin nhắn sẽ bị xóa.
            </p>

            <div className="flex justify-end gap-2 pt-2">
              <button
                onClick={() => setOpenDeleteDialog(false)}
                className="px-4 py-2 text-sm rounded-lg bg-muted hover:bg-accent transition"
              >
                Hủy
              </button>
      ...
      {kickUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-background rounded-2xl p-6 w-[320px] shadow-xl border border-border space-y-4">
            <h3 className="text-lg font-bold">Xóa thành viên?</h3>

            <p className="text-sm text-muted-foreground">
              Bạn có chắc chắn muốn xóa{" "}
              <span className="font-semibold">{kickUser.displayName}</span> ra
              khỏi nhóm?
            </p>
...
const SearchBox = ({ chatId, onResultClick }: any) => {
  const { searchMessages, searchResults, searchLoading } = useChatStore();
