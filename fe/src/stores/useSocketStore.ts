...
export const useSocketStore = create<SocketState>((set, get) => ({
  ...
  connectSocket: () => {
    ...
    socket.on(
      "message-pin-toggled",
      ({ conversationId, messageId, action, pinnedMessage }) => {
        ...
        if (action === "pinned") {
          ...
          // Nếu mình đã từng xoá cuộc trò chuyện sau khi tin nhắn này được gửi, thì không hiện ghim
          if (conversation && currentUserId && conversation.clearedAt) {
            const clearedRecord = conversation.clearedAt.find(
              (c: any) => c.userId === currentUserId,
            );
            if (
    ...
    socket.on(
      "nickname-updated",
      ({ conversationId, targetId, setterId, nickname }) => {
        const chatStore = useChatStore.getState();

        chatStore.updateConversation({
          _id: conversationId,
          participants: (
            chatStore.conversations.find((c) => c._id === conversationId)
              ?.participants || []
          ).map((p) => {
    ...
    socket.on("member-added", ({ conversationId, addedBy, newMembers }) => {
      const chatStore = useChatStore.getState();
      const convo = chatStore.conversations.find(
        (c) => c._id === conversationId,
      );
      if (!convo) {
      ...
      const existingIds = new Set(
        convo.participants.map((p) => p._id.toString()),
      );
